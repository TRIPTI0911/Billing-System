...
const HisabTally = ({ filters }) => {
  ...
  const [showPartyPayments, setShowPartyPayments] = useState(false);
  const [showFactoryPayments, setShowFactoryPayments] = useState(false);
  ...
  const fetchHisabData = async () => {
    ...
    try {
      ...
      if (partyId) {
        const [partyRes, partyPaymentsRes] = await Promise.all([
          axios.get(`http://localhost:5001/api/transactions/summary/party/${partyId}`, { params }),
          axios.get(`http://localhost:5001/api/payments/party_payments/${partyId}`, { params }),
        ]);
        partyData = {
          ...partyRes.data,
          payments: partyPaymentsRes.data || [],
        };
      }

      // Fetch factory summary if factoryId is provided
      if (factoryId) {
        const [factoryRes, factoryPaymentsRes] = await Promise.all([
          axios.get(`http://localhost:5001/api/transactions/summary/factory/${factoryId}`, { params }),
          axios.get(`http://localhost:5001/api/payments/factory_payments/${factoryId}`, { params }),
        ]);
        factoryData = {
          ...factoryRes.data,
          payments: factoryPaymentsRes.data || [],
        };
      }
      ...
      {data?.party && (
        <div className="mb-8">
          ...
          <div className="bg-green-50 p-4 rounded-lg mb-4">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-xl font-semibold text-green-800">Financial Summary</h3>
              <button
                onClick={() => setShowPartyPayments(!showPartyPayments)}
                className="flex items-center text-sm font-medium text-green-800 hover:text-green-900"
              >
                {showPartyPayments ? <ChevronUp className="h-4 w-4 mr-1" /> : <ChevronDown className="h-4 w-4 mr-1" />}
                {showPartyPayments ? 'Hide Payments' : 'Show Payments'}
              </button>
            </div>
                ...
                <table className="min-w-full text-sm">
                  <thead className="bg-green-100 text-green-900">
                    <tr>
                      ...
                      <th className="px-4 py-2 text-left font-semibold">Amount Paid</th>
                      <th className="px-4 py-2 text-left font-semibold">Remarks</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-green-100">
                    {(data.party.payments || []).length === 0 ? (
                      <tr>
                        <td className="px-4 py-3 text-gray-500" colSpan={3}>No party payments found.</td>
                      </tr>
                    ) : (
                      data.party.payments.map((payment) => (
                        <tr key={payment.id}>
                          <td className="px-4 py-2">{formatDate(payment.date)}</td>
                          <td className="px-4 py-2">{formatCurrency(payment.amount_paid)}</td>
      ...
      {data?.factory && (
        <div>
          ...
          <div className="bg-orange-50 p-4 rounded-lg mb-4">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-xl font-semibold text-orange-800">Financial Summary</h3>
              <button
                onClick={() => setShowFactoryPayments(!showFactoryPayments)}
                className="flex items-center text-sm font-medium text-orange-800 hover:text-orange-900"
              >
                {showFactoryPayments ? <ChevronUp className="h-4 w-4 mr-1" /> : <ChevronDown className="h-4 w-4 mr-1" />}
                {showFactoryPayments ? 'Hide Payments' : 'Show Payments'}
              </button>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="text-center">
            ...
            {showFactoryPayments && (
              <div className="mt-4 bg-white rounded-lg border border-orange-200 overflow-hidden">
                <table className="min-w-full text-sm">
                  ...
                  <tbody className="divide-y divide-orange-100">
                    {(data.factory.payments || []).length === 0 ? (
                      <tr>
                        <td className="px-4 py-3 text-gray-500" colSpan={3}>No factory payments found.</td>
                      </tr>
                    ) : (
                      data.factory.payments.map((payment) => (
                        <tr key={payment.id}>
                          <td className="px-4 py-2">{formatDate(payment.date)}</td>
                          <td className="px-4 py-2">{formatCurrency(payment.amount_received)}</td>
                          <td className="px-4 py-2">{payment.remarks || '-'}</td>
                        </tr>
                      ))
                    )}
